const students=[{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
    ...
    function displayStudents(data) {
        ...
        data.forEach(student => {
            ...
            // Create a cell for the combined name and image
            const nameCell = document.createElement('td');
            const img = document.createElement('img');
            img.src = student.img_src;
            img.alt = `${student.first_name} ${student.last_name}'s picture`;
            img.style.width = '50px'; // Set width for image
            img.style.height = '50px'; // Set height for image
            nameCell.appendChild(img); // Add image to the cell
            nameCell.appendChild(document.createTextNode(` ${student.first_name} ${student.last_name}`)); // Add name to the cell
    
            // Create cells for other properties
            ...
            passingCell.textContent = student.passing ? 'Yes' : 'No';
    ...
    function searchStudents() {
        ...
        // Filter the students array based on the search input
        const filteredStudents = students.filter(student => {
            const fullName = `${student.first_name} ${student.last_name}`.toLowerCase();
            const email = student.email.toLowerCase();
    
            // Check if the search input is in the full name or email
...
function sortByMarks() {
    const sortedStudents = students.slice().sort((a, b) => a.marks - b.marks);
    displayStudents(sortedStudents);
}
...
function sortByClass() {
    const sortedStudents = students.slice().sort((a, b) => a.class - b.class);
    displayStudents(sortedStudents);
}

// Sort by Gender and display tables
function sortByGender() {
    // You can implement a similar function as previously shown to separate male and female students
    const maleStudents = students.filter(student => student.gender === 'Male');
    const femaleStudents = students.filter(student => student.gender === 'Female');
    
    // Display male and female students in separate tables
    displayStudents(maleStudents); // You can modify this function to create separate tables
